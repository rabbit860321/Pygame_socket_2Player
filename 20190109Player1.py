import pygame,sys,socket,random,time,math,datetime,sqlite3
import connection
from tkinter import *
from tkinter import ttk
from pygame.locals import *

MY_SERVER_HOST = '192.168.0.2'
MY_SERVER_PORT = 9999
OTHER_HOST = '192.168.0.3'
OTHER_PORT = 8888
...
while GAME_OVER == False:
    ...
    if event.type == pygame.MOUSEBUTTONDOWN:         #選項滑鼠點擊
        if options[0].rect.collidepoint(pygame.mouse.get_pos()):  #按下開始
            ...
            Playing = True
        if options[1].rect.collidepoint(pygame.mouse.get_pos()):  #按下排行榜
            screen.fill((0, 0, 0))
            
            rank = cur.execute("SELECT player,cost_time FROM score ORDER BY cost_time ASC LIMIT 3")

            y = 50
            
            for item in rank:
                rank_text = menu_font.render(str(item),True,(255,255,255)) 
                ...
                if me.count == 99:                       #自己先達到99
                    ...
                    for event in pygame.event.get():
                        if event.type == pygame.QUIT:
                            record = (me.name,timer[0])
                            cur.execute('INSERT INTO score(player,cost_time) VALUES (?,?)',record)
                            conn.commit()
                            pygame.quit()
                            sys.exit()
